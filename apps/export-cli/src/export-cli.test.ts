 ...
 *
 * This is the only test in the repo that runs the glTF exporter at all, and it
 * exists because the app's whole reason to be is a property that nothing else
 * checks: a `.glb` comes out under plain Node, with no browser and no WebGL
 * context. Vitest's default environment is `node`, so the DOM this asserts
 * against is genuinely absent rather than a jsdom stand-in.
 */
...
it('writes a binary glTF for the sample map with no WebGL context', async () => {
  ...
  // Nothing along the way reached for a renderer. The shim installs exactly
  // three globals and this is not one of them.
  expect('WebGLRenderingContext' in globalThis).toBe(false)
})

it('embeds its textures as real PNGs', async () => {
  const glb = await exportSampleMap()
  ...
  // Not decoration. `GLTFExporter` duck-types a `DataTexture` as
  // `image.data !== undefined`, and the canvas implementation this app
  // installs has a `data()` method of its own — see `headless-dom.ts`. Lose
  // that one line and the exporter encodes every texture down the raw-pixel
  // branch, which fails loudly today only by luck.
