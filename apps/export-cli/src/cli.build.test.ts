 ...
 * a real bug in this app hid: `bakedDir()` (`@papercut/fixtures`) used to
 ...
 * `import.meta.resolve('@papercut/fixtures/package.json')` at runtime,
 ...
 * under this package's own tree (`apps/export-cli/node_modules/@papercut
 * /fixtures` is pnpm's real symlink), false of the OS temp dir, which has no
 * such ancestor and fails with `Cannot find package '@papercut/fixtures'`
...
describe('the built CLI, run as its own process', () => {
  it('produces a .glb starting with the glTF magic under plain node', async () => {
    ...
    try {
      ...
      const dir = await mkdtemp(join(tmpdir(), 'papercut-export-cli-build-'))
