...
describe('mergeParams', () => {
  it('carries every host field, not a named few: a panel reading one the merge forgot would show nothing selected', () => {
    const merged = mergeParams({ tool: 'select', spriteName: 'rock', snap: 'half', features: { sketch: { sketchSnap: 'free' } } })
  ...
  it('lets a feature slice sit beside the host fields without shadowing them', () => {
    const merged = mergeParams({ tool: 'terrain', spriteName: 'tree', snap: 'grid', features: { rogue: { tool: 'nope', snap: 'free' } } })
