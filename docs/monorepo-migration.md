...
it is visible only to apps (currently `editor`, and the future `editor-host`), and it must
not reach the `document` layer — the React+Mantine package is the editor's design vocabulary,
