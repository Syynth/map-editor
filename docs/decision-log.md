...
## The feature-module extension surface graduates to a deliverable, on the registry-call shape
- **WHEN:** 2026-09-11
- **PROJECT:** map-editor
- **SYSTEM:** editor-ui
- **SCOPE:** architectural
- **WHAT:** Extensibility stops being only a standing constraint on map [#2](https://github.com/Syynth/map-editor/issues/2) and becomes a deliverable of it. The shape is **B** of three prototyped in [#9](https://github.com/Syynth/map-editor/issues/9) (branch [`prototype/feature-module-shapes`](https://github.com/Syynth/map-editor/tree/prototype/feature-module-shapes)): a small runtime registry a feature module imports and calls at import time — `commands.declare` / `panels.declare` / `keymap.declare` for the static declarations, with handlers riding on an actor the feature exports and the host spawns. Two registries joined by a string id. Rejected: **A**, a folder convention with no API; **C**, an inert manifest object the host interprets. `declare()` must own teardown — it registers against the feature's identity and the host disposes by feature id, rather than handing back disposables the module is trusted to collect.
- **WHY:** B is VS Code's actual shape, which the research in [#5](https://github.com/Syynth/map-editor/issues/5) found is the one this map needs; C is an extrapolation past the reference rather than a finding from it. C's payoff is a types-only import surface, and that only pays off for out-of-tree plugins, which are explicitly out of scope for this map — in-tree, importing a registry singleton costs nothing real. The prototype's Outliner module demonstrated B's failure mode by leaking its palette entry and panel on uninstall, which is why disposal is the registry's responsibility and not the feature author's; a rule nothing enforces is not a rule. This reverses the prototype-era position that "a plugin API invented before its second consumer is an API designed against one example": seeing the three shapes written out showed the certainly-needed part is small and separable from the speculative part.
