...
// along with the app, and every test is plain TypeScript or ESM JavaScript
// with no JSX — in a `packages/*` package, a repo-root `tests/*.test.ts`
// (dependency direction, the gate workflow, and other cross-package
// assertions with no single package to live under), or (since #57) a
// `scripts/*.test.mjs` beside the script it covers — so reaching across for
